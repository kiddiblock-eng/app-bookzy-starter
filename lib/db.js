...
export async function dbConnect() {
  ...
  if (!cached.promise) {
    ...
    const opts = {
      ...
      maxPoolSize: 1, // ✅ Optimisation Serverless : 1 seule connexion suffit
      serverSelectionTimeoutMS: 5000, // Timeout plus court pour ne pas bloquer
    };
    ...
    cached.promise = mongoose.connect(MONGODB_URI, opts).then((mongooseInstance) => {
      ...
      // ─────────────────────────────────────────────────────────────────────────────
      // 🚀 PERFORMANCE : ON REPOUSSE TOUTE TA LOGIQUE LOURDE À PLUS TARD
      // L'utilisateur reçoit la page TOUT DE SUITE.
      // Le serveur s'occupe de la maintenance (les 200 lignes de logique) dans 10s.
      // ─────────────────────────────────────────────────────────────────────────────
      if (!migrationsLaunched) {
        migrationsLaunched = true;
        setTimeout(() => {
          // console.log("⚙️ [Background] Lancement des migrations (Settings, Index, etc)...");
...
// 👇👇👇 ICI C'EST TON ANCIEN CODE (100% PRÉSERVÉ) 👇👇👇
// Je l'ai juste mis dans une fonction pour pouvoir l'appeler plus tard
...
async function runAllYourLogics(db) {
  ...
  try {
    ...
    if (!existingSettings) {
      ...
      // 🔧 MIGRATION: Ajouter les champs manquants (Gemini, Pawapay, etc.)
      const updates = {};
