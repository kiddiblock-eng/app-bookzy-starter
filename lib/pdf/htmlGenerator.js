...
export function generateStyledHTML(data, templateKey = "modern") {
  ...
  // Supprime le premier h2 si son texte correspond au titre du chapitre (évite duplication)
  function cleanChapterContent(content, chapterTitle) {
    if (!content || !chapterTitle) return content;
    
    // Supprimer le premier h2 qui contient le titre du chapitre
    const h2Regex = new RegExp(`<h2[^>]*>\\s*${escapeRegex(chapterTitle)}\\s*</h2>`, 'i');
    const cleaned = content.replace(h2Regex, '');
    
    // Si rien n'a été supprimé, essayer sans les balises exactes
    if (cleaned === content) {
      // Supprimer le premier h2 quel qu'il soit (fallback)
