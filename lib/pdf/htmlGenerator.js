...
// ============================================================================
export function generateStyledHTML(data, templateKey = "modern", langue = "français") {
  ...
  const coverHTML  = buildCover(templateKey, theme, title, author, safeSubtitle, isEn);

  const introLabel    = isEn ? "Introduction" : "Introduction";
  const conclusionLabel = isEn ? "Conclusion" : "Conclusion";

  const tocItems = [
    { num: 1, text: introLabel },
    ...chaptersData.map((ch, i) => ({ num: i + 2, text: ch.title })),
    { num: chaptersData.length + 2, text: conclusionLabel },
  ];
  const tocHTML = buildTOC(tocItems, templateKey, theme, isEn);

  let pageNum  = 3;
  let bodyHTML = buildChapter("00", isEn ? "INTRODUCTION" : "INTRODUCTION", introLabel, intro, templateKey, pageNum++);

  chaptersData.forEach((ch, i) => {
    bodyHTML += buildChapter(
      String(i + 1).padStart(2, '0'),
      isEn ? `CHAPTER ${i + 1}` : `CHAPITRE ${i + 1}`,
      ch.title,
  ...
  bodyHTML += buildChapter("∞", isEn ? "CONCLUSION" : "CONCLUSION", conclusionLabel, conclusion, templateKey, pageNum);
...
// ============================================================================
function buildCover(key, theme, title, author, subtitle, isEn) {
  const auth = author || (isEn ? "Author" : "Auteur");
  const byLabel = isEn ? "By" : "Par";
  ...
  const covers = {
    ...
    <div class="cover-page luxe-cover">
      ...
      <div class="cover-content">
        ...
        <div class="luxe-author"><span class="author-label">${byLabel}</span><span class="author-name">${auth}</span></div>
      </div>
    </div>`,
    ...
    <div class="cover-page minimal-cover">
      <div class="cover-deco"><div class="minimal-grid"></div></div>
      <div class="cover-content">
...
// ============================================================================
function buildTOC(items, key, theme, isEn) {
  ...
  const tocs = {
    ...
    luxe: `<div class="toc-page toc-luxe">
      <h1 class="toc-title">${isEn ? "Table of Contents" : "Table des Matières"}</h1>
    ...
    modern: `<div class="toc-page toc-modern">
      <h1 class="toc-title">${isEn ? "Contents" : "Sommaire"}</h1>
    ...
    educatif: `<div class="toc-page toc-educatif">
      <h1 class="toc-title">${isEn ? "📚 Program" : "📚 Programme"}</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    energie: `<div class="toc-page toc-energie">
      <h1 class="toc-title">${isEn ? "⚡ ROADMAP" : "⚡ ROADMAP"}</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    minimal: `<div class="toc-page toc-minimal">
      <h1 class="toc-title">${isEn ? "Index" : "Index"}</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    creative: `<div class="toc-page toc-creative">
      <h1 class="toc-title">${isEn ? "✨ On the Menu" : "✨ Au Menu"}</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    nature: `<div class="toc-page toc-nature">
      <h1 class="toc-title">${isEn ? "🌿 Journey" : "🌿 Parcours"}</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    futuriste: `<div class="toc-page toc-futuriste">
      <h1 class="toc-title">◢ ${isEn ? "NAVIGATION" : "NAVIGATION"} ◣</h1>
      <div class="toc-list">${items.map(it=>`
    ...
    wellness: `<div class="toc-page toc-wellness">
      <h1 class="toc-title">${isEn ? "🌸 Your Journey" : "🌸 Votre Parcours"}</h1>
      <div class="toc-list">${items.map(it=>`
...
function buildCSS(theme, key) {
  const needsInterFallback = ['luxe', 'fashion', 'retro'].includes(key);
  return `
    @import url('${getFontUrl(theme.font)}');
    ${needsInterFallback ? `@import url('${getFontUrl('Inter')}');` : ''}

    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: '${theme.font}', "Noto Color Emoji", sans-serif; color: #1e293b; font-size: 18px; line-height: 1.8; background: white; margin: 0; padding: 0; }
    @page { size: A4; margin: 20mm 15mm 30mm 15mm; }
    @page :first { size: A4; margin: 0; padding: 0; }
    .start-chapter { break-before: page; margin-top: 0; padding-top: 0; }
    .toc-page { page-break-after: always; break-after: page; padding:60px 40px; }
    ${needsInterFallback ? `.section-content, .section-content p, .section-content li, .section-content td { font-family: 'Inter', sans-serif !important; }` : ''}
    /* ===== COUVERTURE BASE ===== */
    .cover-page { width:210mm; height:297mm; display:flex; align-items:center; justify-content:center; text-align:center; padding:0; margin:0; page-break-after:always; break-after:page; position:relative; overflow:hidden; box-sizing:border-box; }
    .cover-deco { position:absolute; inset:0; z-index:1; pointer-events:none; }
    .cover-content { position:relative; z-index:10; padding:40px 60px; width:100%; background:transparent; }
    ...
    .modern-grid-3d { position:absolute; inset:0; background-image:linear-gradient(rgba(255,255,255,.1) 2px,transparent 2px),linear-gradient(90deg,rgba(255,255,255,.1) 2px,transparent 2px); background-size:80px 80px; opacity:.5; }
    ...
    .modern-shapes { position:absolute; inset:0; overflow:hidden; z-index:1; }
    ...
    .energie-circles { position:absolute; inset:0; z-index:1; }
    ...
    .cb { position:absolute; border-radius:30% 70% 70% 30% / 30% 30% 70% 70%; opacity:.15; z-index:1; }
    .cb.blob-1 { width:400px; height:400px; background:${theme.accent}; top:-150px; right:-150px; }
    .cb.blob-2 { width:300px; height:300px; background:white; bottom:-100px; left:-100px; border-radius:70% 30% 30% 70% / 70% 70% 30% 30%; }
    .cb.blob-3 { width:200px; height:200px; background:${theme.secondary}; top:45%; left:15%; opacity:.1; }
    .cb.blob-4 { width:150px; height:150px; background:white; bottom:30%; right:20%; opacity:.08; }
    ...
    .creative-title { font-size:58px; font-weight:900; line-height:1.15; margin:0 70px 40px; color:white; text-shadow:0 2px 20px rgba(0,0,0,.2); }
    ...
    .tech-circuits { position:absolute; inset:0; z-index:1; }
    ...
    .futuriste-particles { position:absolute; inset:0; z-index:1; }
    ...
    .afrique-circles { position:absolute; inset:0; z-index:1; }
    ...
    /* ===== WELLNESS ===== */
    .wellness-cover { background:linear-gradient(135deg,#f3e8ff 0%,#e9d5ff 50%,#c4b5fd 100%); color:#2e1065; width:210mm; height:297mm; }
    .wellness-circles { position:absolute; inset:0; z-index:1; }
    ...
    .toc-luxe .toc-item { padding:15px 0; border-bottom:1px solid ${theme.secondary}; align-items:center; display:flex; gap:15px; }
    .toc-luxe .toc-num-roman { font-family:'Playfair Display',serif; font-size:22px; font-weight:700; color:${theme.secondary}; min-width:50px; flex-shrink:0; }
    .toc-luxe .toc-dots { flex:1; height:1px; background:repeating-linear-gradient(90deg,${theme.secondary} 0,${theme.secondary} 4px,transparent 4px,transparent 8px); margin:0 15px; min-width:20px; }
    .toc-luxe .toc-text { font-family:'Playfair Display',serif; font-size:17px; font-style:normal; font-weight:600; color:${theme.primary}; flex-shrink:0; max-width:60%; text-align:right; }
    ...
    .template-creative .chapter-title { font-size:42px; font-weight:900; color:${theme.primary}; line-height:1.3; }
