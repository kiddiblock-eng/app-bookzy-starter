...
export async function getAIText(feature, prompt, maxTokens = 8000) {
  ...
  if (provider === "gemini") {
    ...
    // ✅ FALLBACK AUTOMATIQUE — du plus rapide au plus stable
    // Pro en dernier recours = jamais surchargé
    const fallbackChain = [
      model,
    ...
    for (const fallbackModel of fallbackChain) {
      try {
        console.log(`🤖 Gemini: essai avec ${fallbackModel}...`);
        const genModel = client.getGenerativeModel({ model: fallbackModel });
        const result = await genModel.generateContent({
          contents: [{ role: "user", parts: [{ text: prompt }] }],
          safetySettings,
        });
        console.log(`✅ Gemini: succès avec ${fallbackModel}`);
        return result.response.text();
      } catch (err) {
        // 404 = modèle obsolète → passer au suivant sans retry
        const isFatal = err.status === 404 ||
          err.message?.includes("404") ||
          err.message?.includes("no longer available");

        // 503 = surchargé → passer au suivant
        ...
        if ((isFatal || isHighDemand) && fallbackChain.indexOf(fallbackModel) < fallbackChain.length - 1) {
          console.warn(`⚠️ Gemini ${fallbackModel} indispo — passage au modèle suivant...`);
          continue;
        }
        throw err;
      }
    ...
    // ✅ Si tous les modèles sont KO → retourner null (pas throw)
    // La route gèrera le cas "highDemand" côté client
    console.warn("⚠️ Tous les modèles Gemini sont surchargés — retour null.");
    return null;
  }
