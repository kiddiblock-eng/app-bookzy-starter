// lib/shop-templates/GlassTemplate.jsx

export default function GlassTemplate({ shop, products, onProductClick }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-pink-500/20 rounded-full blur-[100px] pointer-events-none" />
          ...
          {shop.logo ? (
            <div className="relative inline-block">
              <img
                src={shop.logo}
                alt={shop.name}
                className="w-24 h-24 rounded-full mx-auto object-cover border border-white/20 shadow-2xl"
              />
              <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-white/10 to-transparent" />
            </div>
          ) : (
            <div className="w-24 h-24 rounded-full mx-auto bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center shadow-2xl">
          ...
          <div className="flex items-center justify-center gap-3 mt-4">
            {shop.socials.instagram && (
              <a href={`https://instagram.com/${shop.socials.instagram}`} target="_blank" rel="noopener noreferrer" 
                className="w-10 h-10 rounded-full bg-white/10 backdrop-blur border border-white/20 flex items-center justify-center text-white/70 hover:bg-white/20 hover:text-white transition-all">
        ...
        <div className="space-y-4">
          {products?.map((product) => (
            <div
              ...
              className="group bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex gap-4 cursor-pointer 
          ...
          {(!products || products.length === 0) && (
            <div className="text-center py-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl">
              ...
              <p className="text-sm text-white/50">Aucun produit disponible</p>
            </div>
          )}
        </div>
      </div>
