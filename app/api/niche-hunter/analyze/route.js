...
export async function POST(req) {
  try {
    ...
    if (niche.analyzed && niche.analysis) return NextResponse.json({ success: true, data: { niche } });
    ...
    if (canUseQuota) {
      ...
      if (balance < 1) {
        return NextResponse.json({ success: false, quotaExceeded: true, insufficientCredits: true, plan: userDoc.plan, balance, message: "Quota journalier épuisé et crédits insuffisants." }, { status: 402 });
      }
...
`;
    ...
    console.log(`🚀 Analyse de "${niche.title}" (Marché: ${analysis.targetMarket || 'africa'}) - 4 appels parallèles avec retry...`);
...
JSON STRICT (sans apostrophes dans les textes) :
{
  ...
  "publicCible": "A qui vendre exactement (2-3 phrases max, ton naturel)"
    ...
    // ✅ 4 appels parallèles avec retry automatique (3 tentatives chacun)
    const [raw1, raw2, raw3, raw4] = await Promise.all([
      getAITextWithRetry("nicheAnalyze", prompt1, 1500, 3),
      getAITextWithRetry("nicheAnalyze", prompt2, 1200, 3),
      getAITextWithRetry("nicheAnalyze", prompt3, 1200, 3),
      getAITextWithRetry("nicheAnalyze", prompt4, 1200, 3)
    ]);
