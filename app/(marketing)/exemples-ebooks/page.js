    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
          <div>
            <h3 className="font-bold text-slate-900 text-sm sm:text-base">{ebook.title}</h3>
            <p className="text-xs text-slate-400">Template {TEMPLATES[ebook.template].label} · Généré par Bookzy</p>
          </div>
...
function EbookCard({ ebook, index, onPreview }) {
  ...
  return (
    <div
      ...
      style={{
        background: hovered ? "#131620" : "#0F1117",
        border: `1px solid ${hovered ? "#2A3050" : "#1E2235"}`,
        borderRadius: 16,
        padding: "28px 24px",
        display: "flex",
    ...
    >
      ...
      <div style={{ display:"flex",gap:18,alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1,minWidth:0 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:10 }}>
            <span style={{ fontFamily:"monospace",fontSize:10,color:"#A09080",fontWeight:700 }}>
          ...
          <h3 style={{ margin:"0 0 6px",color:"#1A1A1A",fontWeight:800,fontSize:15,lineHeight:1.3,letterSpacing:"-0.3px" }}>
            {ebook.title}
          </h3>
          <p style={{ margin:"0 0 14px",color:"#8A8070",fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.1em" }}>
            {ebook.niche}
          </p>
          ...
          <div style={{ display:"flex",gap:16 }}>
            <div style={{ display:"flex",alignItems:"center",gap:5 }}>
              <Clock size={11} color="#2A3050" />
              <span style={{ fontSize:11,color:"#6B6460",fontWeight:600 }}>{ebook.time}s</span>
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:5 }}>
              <Eye size={11} color="#2A3050" />
              <span style={{ fontSize:11,color:"#6B6460",fontWeight:600 }}>{ebook.pages} pages</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        <div>
          <p style={{ margin:"0 0 2px",color:"#A09080",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.12em" }}>
            Valeur marché
          </p>
          <p style={{ margin:0,color:"#2C4A8A",fontWeight:900,fontSize:18,letterSpacing:"-0.5px" }}>
            {ebook.price.toLocaleString()} <span style={{ fontSize:11,fontWeight:600,color:"#8A8070" }}>{ebook.currency}</span>
          </p>
        </div>
        <button
          ...
          style={{
            ...
            gap:6,
            padding:"9px 16px",
            background: hovered ? "#1A1F30" : "#131620",
            border:"1px solid #C8BFB0",
            borderRadius:8,
            color:"#2C4A8A",
            fontSize:12,
...
export default function EbooksVitrinePage() {
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#F5F2ED",overflowX:"hidden",fontFamily:"'Helvetica Neue', Helvetica, Arial, sans-serif" }}>

      {/* ── HEADER ── */}
      <header style={{ borderBottom:"1px solid #C8BFB0",padding:"0 24px" }}>
        <div style={{ maxWidth:1100,margin:"0 auto",height:60,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <Link href="/" style={{ textDecoration:"none",fontWeight:900,fontSize:20,color:"#1A1A1A",letterSpacing:"-0.5px" }}>
            Bookzy<span style={{ color:"#2C4A8A" }}>.</span>
          </Link>
          <Link
            href="/auth/register"
            style={{
      ...
      <section style={{ maxWidth:1100,margin:"0 auto",padding:"80px 24px 60px",textAlign:"center" }}>
        ...
        <h1 style={{ margin:"0 0 20px",color:"#1A1A1A",fontWeight:900,fontSize:"clamp(32px,6vw,60px)",lineHeight:1.05,letterSpacing:"-2px" }}>
          {EBOOKS.length} ebooks créés.<br />
          <span style={{ color:"#2C4A8A" }}>En moins de 70 secondes.</span>
        </h1>
        ...
        {/* Stats */}
        <div style={{ display:"flex",justifyContent:"center",gap:0,marginTop:48,borderTop:"1px solid #C8BFB0",borderBottom:"1px solid #C8BFB0",padding:"28px 0" }}>
          {[
            ...
            <div key={i} style={{ flex:1,textAlign:"center",borderRight:i<3?"1px solid #C8BFB0":"none",padding:"0 24px" }}>
              <p style={{ margin:"0 0 6px",fontWeight:900,fontSize:"clamp(20px,3vw,30px)",color:"#1A1A1A",letterSpacing:"-1px" }}>{stat.value}</p>
      ...
      {/* ── CTA FINAL ── */}
      <section style={{ borderTop:"1px solid #C8BFB0",background:"#EDE8E0" }}>
        <div style={{ maxWidth:600,margin:"0 auto",padding:"72px 24px",textAlign:"center" }}>
          ...
          <h2 style={{ margin:"0 0 16px",color:"#1A1A1A",fontWeight:900,fontSize:"clamp(24px,4vw,38px)",letterSpacing:"-1px",lineHeight:1.1 }}>
            Crée ton ebook en<br /><span style={{ color:"#2C4A8A" }}>moins d'une minute.</span>
          </h2>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop:"1px solid #1A1F30",padding:"24px",textAlign:"center" }}>
        <p style={{ margin:0,fontSize:11,color:"#C8BFB0",fontWeight:600 }}>
