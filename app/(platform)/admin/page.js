...
export default function DashboardClient({ initial }) {
  ...
  // --- ETATS DE DONNEES (initialisés depuis le serveur, fallback vides) ---
  ...
  const [recentUsers, setRecentUsers] = useState(initial?.recentUsers || []);
  const [recentEbooks, setRecentEbooks] = useState(initial?.recentEbooks || []);
  const [notifications, setNotifications] = useState(initial?.notifications || []);
  const [goals, setGoals] = useState(initial?.goals || []);
  const [topUsers, setTopUsers] = useState(initial?.topUsers || []);
  const [activityData, setActivityData] = useState(initial?.activityData || []);
  const [revenueData, setRevenueData] = useState(initial?.revenueData || []);
  const [performanceData, setPerformanceData] = useState(initial?.performanceData || []);
  ...
  useEffect(() => {
    if (!initial?.ok) {
      // Pas de données pré-chargées par le serveur → fallback : on charge côté client (ancien comportement, zéro régression)
      setLoading(true);
      loadAllData().then(() => { setLoading(false); mountedRef.current = true; });
    } else {
      // Données déjà fournies par le serveur → affichage immédiat, rien à recharger au montage
