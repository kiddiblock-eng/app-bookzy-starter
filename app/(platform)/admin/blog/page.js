...
export default function AdminBlogsPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center gap-4 text-neutral-500">
             ...
             <div className="flex p-1 bg-white border border-neutral-200 rounded-lg">
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold rounded-md transition-all flex items-center gap-2 ${
                    tab === "list" ? "bg-neutral-900 text-white shadow-sm" : "text-neutral-500 hover:text-neutral-900"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold rounded-md transition-all flex items-center gap-2 ${
                    tab === "create" ? "bg-emerald-600 text-white shadow-sm" : "text-neutral-500 hover:text-neutral-900"
                  }`}
                >
             ...
             <div className="space-y-8">
               <div className="relative max-w-md">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-neutral-200 text-neutral-900 text-sm rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-neutral-900 focus:outline-none transition-all placeholder:text-neutral-400"
                  />
               </div>
               ...
               {filteredBlogs.length === 0 ? (
                  <div className="text-center py-20 border border-dashed border-neutral-200 rounded-xl">
                     ...
                     <h3 className="text-sm font-bold text-neutral-500 uppercase tracking-wide">Aucun article</h3>
                  </div>
               ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                     {filteredBlogs.map((blog) => (
                        <div key={blog._id} className="group flex flex-col">
                           ...
                           <div className="h-48 rounded-2xl bg-neutral-100 relative overflow-hidden mb-4 border border-neutral-200 group-hover:border-emerald-200 transition-all">
                              {blog.cover ? (
                                 <img src={blog.cover} alt={blog.title} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                              ) : (
                                 <div className="w-full h-full flex items-center justify-center text-neutral-300"><ImageIcon size={32}/></div>
                              )}
                           </div>

                           <div className="flex-1 flex flex-col">
                              <h3 className="text-lg font-bold text-neutral-900 mb-2 leading-tight group-hover:text-emerald-600 transition-colors">
                                 {blog.title}
                              </h3>
                              ...
                              <div className="flex items-center justify-between pt-2 border-t border-neutral-200">
                                 <span className="text-xs text-neutral-500 font-mono">{new Date(blog.createdAt).toLocaleDateString()}</span>
                                 <div className="flex gap-3">
                                    <a href={`/blog/${blog.slug}`} target="_blank" className="text-neutral-500 hover:text-neutral-900 transition-colors" title="Voir"><Eye size={14} /></a>
                                    <a href={`/admin/blog/edit/${blog._id}`} className="text-neutral-500 hover:text-emerald-600 transition-colors" title="Éditer"><Edit3 size={14} /></a>
                                    <button onClick={() => deleteBlog(blog._id)} className="text-neutral-500 hover:text-red-500 transition-colors" title="Supprimer"><Trash2 size={14} /></button>
                                 </div>
                              </div>
                           </div>
                        </div>
                ...
                <form onSubmit={handleCreateBlog} className="space-y-16">
                   ...
                   <div>
                      <div className="mb-8">
                         ...
                         <p className="text-sm text-neutral-500 mt-1">Informations de base de l'article</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
                        ...
                        <div>
                           <label className="text-xs font-bold text-neutral-500 uppercase mb-2 block">Image Couverture (URL)</label>
                           <div className="relative">
                              ...
                              <input
                                 ...
                                 placeholder="https://..."
                                 className="w-full bg-white border border-neutral-200 text-neutral-700 text-sm rounded-lg pl-10 pr-4 py-4 focus:ring-2 focus:ring-neutral-900 focus:outline-none placeholder:text-neutral-400"
                              />
                           </div>
                        </div>

                        <div className="md:col-span-2">
                           <label className="text-xs font-bold text-neutral-500 uppercase mb-2 block">Extrait (Résumé)</label>
                           <textarea
                              rows={2}
                              value={excerpt}
                              onChange={(e) => setExcerpt(e.target.value)}
                              placeholder="Court résumé pour les cartes..."
                              className="w-full bg-white border border-neutral-200 text-neutral-700 text-sm rounded-lg p-4 focus:ring-2 focus:ring-neutral-900 focus:outline-none resize-none placeholder:text-neutral-400"
                           />
                        </div>
                        ...
                        <div className="md:col-span-2">
                           ...
                           <textarea
                              ...
                              className="w-full bg-white border border-neutral-200 text-neutral-700 font-mono text-sm rounded-lg p-4 focus:ring-2 focus:ring-neutral-900 focus:outline-none placeholder:text-neutral-400"
                              required
                           />
                        </div>
                      </div>
                   ...
                   <div className="pt-8 border-t border-neutral-200">
                      <div className="mb-8">
                         <h3 className="text-lg font-bold text-neutral-900 flex items-center gap-2">
                            <Target size={18} className="text-emerald-600" /> SEO & Métadonnées
                         </h3>
                         <p className="text-sm text-neutral-500 mt-1">Optimisation pour les moteurs de recherche</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-8">
                         <div>
                            ...
                            <div className="flex justify-between mt-2 px-1">
                               <span className="text-[10px] text-neutral-500">Recommandé : 50-60 caractères</span>
                               <span className={`text-[10px] font-mono ${seoTitle.length > 60 ? 'text-red-500' : 'text-emerald-600'}`}>{seoTitle.length}/60</span>
                            </div>
                         </div>
                         ...
                         <div>
                            ...
                            <div className="flex justify-between mt-2 px-1">
                               <span className="text-[10px] text-neutral-500">Recommandé : 150-160 caractères</span>
                               <span className={`text-[10px] font-mono ${seoDescription.length > 160 ? 'text-red-500' : 'text-emerald-600'}`}>{seoDescription.length}/160</span>
                            </div>
                         </div>
                         ...
                         <div>
                            ...
                            <div className="relative">
                               ...
                               <input type="text" value={seoKeywords} onChange={(e) => setSeoKeywords(e.target.value)} placeholder="tag1, tag2, tag3" className="w-full bg-white border border-neutral-200 text-neutral-700 text-sm rounded-lg pl-10 pr-4 py-4 focus:ring-2 focus:ring-neutral-900 focus:outline-none placeholder:text-neutral-400" />
                            </div>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-end gap-6 pt-8 border-t border-neutral-200 sticky bottom-0 bg-neutral-50/95 backdrop-blur py-4 z-10">
                      <button type="button" onClick={() => setTab("list")} className="text-sm font-medium text-neutral-500 hover:text-neutral-900 transition-colors">
                         Annuler
                      </button>
                      <button type="submit" disabled={saving} className="px-8 py-3 bg-neutral-900 hover:bg-neutral-800 text-white font-bold rounded-xl disabled:opacity-50 flex items-center gap-2 transition-all">
