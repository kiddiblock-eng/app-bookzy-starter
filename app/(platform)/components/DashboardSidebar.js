  ...
  const CollapsedSidebar = () => (
    <div className="flex flex-col h-full bg-white items-center py-3">
      <Link href="/" className="mb-1 h-9 flex items-center justify-center">
        <img src="/logonew.png" alt="Bookzy" className="h-7 w-auto object-contain" />
      </Link>
      ...
      <div className="flex flex-col h-full bg-white">
        ...
        <div className="flex items-center justify-between px-3 h-14 shrink-0">
          <Link href="/" onClick={close} className="flex items-center">
            <img src="/logonew.png" alt="Bookzy" className="h-9 w-auto object-contain" />
          </Link>
          <button
        ...
        <nav className="flex-1 overflow-y-auto px-2 pb-2">
          {/* Action principale */}
          <Link href={PRIMARY.href} onClick={close} className={`${itemCls(primaryOn)} mb-1`}>
              ...
              <div className="flex flex-col gap-0.5">
                {section.items.map(({ label, href, icon: Icon, exact, accordion, external }) => {
                  ...
                  if (accordion === "niche" || accordion === "analyseur") {
                    ...
                    return (
                      <div key={label}>
                        <button onClick={() => setAcc(!accOpen)} className={`${itemCls(on)} w-full`}>
                  ...
                  return (
                    <Link key={href} href={href} onClick={close} className={itemCls(on)}>
                      <Icon className={iconCls(on)} />
                      <span className="flex-1">{label}</span>
                    </Link>
                  );
