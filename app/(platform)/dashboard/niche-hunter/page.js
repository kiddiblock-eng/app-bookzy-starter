...
import { Search, ArrowRight, ChevronRight, AlertCircle, Gem, TrendingUp, Flame, Trophy } from "lucide-react";
...
const LOADING_STEPS = [
  ...
  { label: "Détection des produits qui cartonnent...", pct: 60 },
  { label: "Génération des idées d'ebooks...", pct: 80 },
  { label: "Calcul des scores de marché...", pct: 95 },
];

const LOADING_MESSAGES = [
  ["Préparation de ton analyse...", "On scrute le marché pour toi 🔍"],
  ["On analyse les pubs actives...", "Les données arrivent en temps réel 📡"],
  ["Détection des produits winners...", "On cherche ce qui cartonne vraiment 🎯"],
  ["L'IA analyse les opportunités...", "Presque prêt pour toi 🧠"],
...
const TELEGRAM_ICON = (<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>);
const INSTAGRAM_ICON = (<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>);
const BOUTIQUE_ICON = (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>);
...
export default function NicheHunterPage() {
  ...
  const handlePaysClick = (p) => {
    if (p === "Autre pays") { setPays(""); setShowAutreInput(true); }
    else { setPays(pays === p ? "" : p); setShowAutreInput(false); setAutrePaysInput(""); }
  };

  const paysFinal = showAutreInput ? autrePaysInput.trim() : pays;

  const startLoadingAnimation = () => {
    let stepIdx = 0, msgIdx = 0, pct = 0;
    intervalRef.current = setInterval(() => {
      pct += Math.random() * 8 + 2;
      if (pct > 96) pct = 96;
      setLoadingPct(Math.round(pct));
      const targetPct = LOADING_STEPS[stepIdx]?.pct || 95;
      if (pct >= targetPct && stepIdx < LOADING_STEPS.length - 1) { stepIdx++; setLoadingStep(stepIdx); }
      if (msgIdx < LOADING_MESSAGES.length - 1 && pct > (msgIdx + 1) * 20) { msgIdx++; setLoadingMsg(msgIdx); }
    }, 400);
  };
  ...
  const handleLaunch = async () => {
    ...
    setStep(3); setError(""); setQuotaExceeded(false);
            ...
            {fbStats && niches.length > 0 && (
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 mb-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                  {[
                    [fbStats.totalAds?.toLocaleString(), "pubs analysées"],
                    [fbStats.uniqueAdvertisers?.toLocaleString(), "annonceurs actifs"],
...
function NicheCard({ niche, analysisId, router }) {
  ...
  const getBadge = (badge) => {
    ...
    return { label: "Produit winner", icon: <Flame className="w-3 h-3" />, style: "bg-orange-50 text-orange-700 border-orange-200" };
  };
