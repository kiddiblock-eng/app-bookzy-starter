...
export default function DashboardHome() {
  ...
  const [stats, setStats] = useState({ total: 0, kits: 0, enCours: 0, progression: 0 });
      ...
      try {
        ...
        if (resEbooks.ok) {
          ...
          if (data.success && Array.isArray(data.ebooks)) {
            ...
            const total = data.ebooks.length;
            const kits = data.ebooks.filter(e => e.fileUrl).length;
            ...
            setStats({
              ...
              kits,
              enCours,
              progression: Math.min((total / 5) * 100, 100),
            });
          }
        }
      } catch (err) {
  ...
  const userName = user 
    ? (user.displayName || `${user.firstName || ""} ${user.lastName || ""}`.trim() || "Créateur")
    : "Créateur";
  const niveau = stats.total < 3 ? "Débutant" : stats.total < 5 ? "Intermédiaire" : "Expert";
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-neutral-600 font-medium">Chargement de votre espace...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="flex-1 space-y-6">
                <div>
                  ...
                  <p className="text-base sm:text-lg text-slate-300 max-w-xl">
                    Préparez-vous à lancer votre prochain produit gagnant en un temps record.
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <a
                    ...
                    className="group inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm transition-all shadow-lg active:scale-[0.98]"
                  >
                  ...
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-white/10 border border-white/20 hover:bg-white/20 text-white rounded-xl font-bold text-sm transition-all"
                  >
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-white flex items-center gap-2">
                      ...
                      <div className="w-6 h-6 text-yellow-500">
                        {stats.total < 5 ? <Star className="w-full h-full fill-yellow-500" /> : <Crown className="w-full h-full fill-yellow-500" />}
                      </div>
                    </p>
                  </div>
                  <span className="text-3xl font-black text-cyan-400">{Math.round(stats.progression)}%</span>
                </div>
                
                <div className="relative h-2.5 bg-slate-700 rounded-full overflow-hidden mb-2">
                  <div
                    className="absolute inset-0 bg-cyan-400 rounded-full transition-all duration-700 shadow-lg shadow-cyan-400/50"
          ...
          <section className="lg:col-span-2 space-y-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{stats.total} projet(s) créé(s) au total</p>
              </div>
              {ebooks.length > 0 && (
                <a
                  href="/dashboard/projets"
                  className="group flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"
                >
          ...
          <aside className="space-y-4">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              ...
              <div className="space-y-2">
                <StatRow label="Projets Terminés" value={stats.kits} icon={CheckCircle2} color="text-emerald-600" />
                <StatRow label="Projets En Cours" value={stats.enCours} icon={Clock} color="text-amber-600" />
...
function QuickLink({ href, icon: Icon, title, subtitle, color }) {
  return (
    <a
      href={href}
      className="group bg-white border border-slate-200 rounded-xl p-4 hover:shadow-lg hover:border-indigo-300 transition-all flex items-center gap-4 hover:-translate-y-0.5"
    >
    ...
    <div className="group bg-white border border-neutral-200 rounded-xl p-4 hover:shadow-md transition-all flex items-center justify-between">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-sm text-neutral-900 truncate group-hover:text-indigo-600 transition-colors">
            {ebook.title}
          </h3>
      ...
      <a
        href={`/dashboard/fichiers/${ebook._id}`}
        className="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 rounded-lg font-medium text-xs transition-all flex-shrink-0"
      >
...
function EmptyState() {
  return (
    <div className="relative bg-white border-2 border-dashed border-neutral-300 rounded-xl p-8 text-center">
      ...
      <p className="text-neutral-600 mb-4 max-w-sm mx-auto text-sm">
        Lancez votre premier projet d'eBook pour voir l'activité récente.
      </p>
      <a
        href="/dashboard/projets/nouveau"
        className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-indigo-600 text-white rounded-xl font-bold text-sm transition-all shadow-lg"
      >
        <Rocket className="w-4 h-4" />
        Créer maintenant
      </a>
