  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8FAFC]">
        ...
        <p className="mt-4 text-slate-400 font-medium text-sm animate-pulse">Synchronisation de la librairie...</p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
             ...
             <div className="bg-white p-1 rounded-xl border border-slate-200 shadow-sm flex items-center px-3 focus-within:ring-2 focus-within:ring-indigo-100 transition-all col-span-2 md:col-span-1">
                ...
                <input 
                  ...
                  className="w-full bg-transparent outline-none text-sm font-medium placeholder:text-slate-400 h-full"
                />
             </div>
          </div>
        )}
             ...
             <div className="flex gap-1 bg-slate-100 p-1 rounded-lg overflow-x-auto max-w-full no-scrollbar">
                {['all', 'terminé', 'en cours', 'erreur'].map(status => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-md text-xs font-bold capitalize transition-all whitespace-nowrap ${filterStatus === status ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                  >
         ...
         {!isList && (
            <div className="flex justify-between items-start mb-3">
                <div className="text-xs text-slate-400 font-medium flex items-center gap-1">
                    <Clock className="w-3 h-3"/> {new Date(projet.createdAt).toLocaleDateString()}
                </div>
            </div>
         )}
